...
public class NoiseMaker : MonoBehaviour
{
    ...
    //Plays at transform.
    public void Play(int clipPool, bool interrupt)
    {
        if (GetClip(clipPool, out AudioClip clipWanted) == false) return;
        PlaySoundEffectAtPoint(clipWanted, transform.position, interrupt, audioClipPools[clipPool]);

    }
    public void Play(int clipPool)
    {
        if (GetClip(clipPool, out AudioClip clipWanted) == false) return;
        PlaySoundEffectAtPoint(clipWanted, transform.position, false, audioClipPools[clipPool]);
    }
